...
- `oracle session clean --hours <n>` now mirrors `oracle status clear` for pruning cached runs, and `oracle status` prints a tip pointing to it—use that alias instead of manipulating `~/.oracle` manually.
