...
Concierge runs in browser mode only, with two execution paths:
...
Concierge routes the assembled prompt bundle through ChatGPT (or Gemini web). The CLI writes the same session metadata/logs and, by default, pastes the payload into ChatGPT via a temporary Chrome profile (manual-login mode can reuse a persistent automation profile).

`--preview` renders the composed prompt, lists which files would be uploaded vs inlined, and shows the bundle location when bundling is enabled, without launching Chrome.

## Quick example: browser mode with custom cookies

```bash
# Minimal inline-cookies flow: keep ChatGPT logged in without Keychain
jq '.' ~/.concierge/cookies.json  # file must contain CookieParam[]
concierge \
  --browser-inline-cookies-file ~/.concierge/cookies.json \
  --model "GPT-5.2 Pro" \
  -p "Run the UI smoke" \
  --file "src/**/*.ts" --file "!src/**/*.test.ts"
```

`~/.concierge/cookies.json` should be a JSON array shaped like:
...
3. **Session integration** – browser sessions use the normal log writer, add `mode: "browser"` plus `browser.config/runtime` metadata, and log the Chrome PID/port so `concierge session <id>` (or `concierge status <id>`) shows a marker for the background Chrome process.
4. **Usage accounting** – we estimate input tokens with the GPT tokenizer and estimate output tokens via `estimateTokenCount`. `concierge status` therefore shows comparable token/timing info even though the call ran through the browser.
...
- `--browser-timeout`, `--browser-input-timeout`: `2400s (40m)`/`30s` defaults. Durations accept `ms`, `s`, `m`, or `h` and can be chained (`1h2m10s`).
...
- **Thinking time**: defaults to `extended` for Thinking/Pro models. Configure via `browser.thinkingTime` in `~/.concierge/config.json` (options: `light`, `standard`, `extended`, `heavy`). Skipped automatically for image generation since the Images page has no thinking time selector.
...
All options are persisted with the session so reruns (`concierge --exec-session <id>`) reuse the same automation settings.

### Manual login mode (persistent profile, no cookie copy)

Use `--browser-manual-login` when cookie decrypt is blocked (e.g., Windows app-bound cookies) or you prefer to sign in explicitly. You can also make it the default via `browser.manualLogin` in `~/.concierge/config.json`.
...
- Cookie copy is skipped by default in this mode. To automate manual-login runs, set `browser.manualLoginCookieSync=true` in `~/.concierge/config.json` to seed the persistent profile from your existing Chrome cookies; inline cookies apply when cookie sync is enabled.
- If Chrome is already running with that profile and DevTools remote debugging enabled (see `DevToolsActivePort` in the profile dir), you can reuse it instead of relaunching by pointing Concierge at it with `--remote-chrome <host:port>`.

## Remote Chrome Sessions (headless/server workflows)

Concierge can reuse an already-running Chrome/Edge instance on another machine by tunneling over the Chrome DevTools Protocol. This is handy when:
...
- Keep this browser window open and signed into ChatGPT; Concierge will reuse that session and **will not** copy cookies over the wire.

### 2. Point Concierge at the remote browser

From the machine running `concierge`:
...
- Local-only flags like `--browser-headless`, `--browser-hide-window`, `--browser-keep-browser`, and `--browser-chrome-path` are ignored because Concierge no longer launches Chrome. You still get verbose logging, model switching, attachment uploads, and markdown capture.
...
- Concierge opens a dedicated CDP target (new tab) for each run and closes it afterward so your existing tabs stay untouched.
- Attachments are transferred via CDP: Concierge reads each file locally, base64-encodes it, and uses `DataTransfer` inside the remote browser to populate the upload field. Files larger than 20 MB are rejected to keep CDP messages reasonable.
- When the remote WebSocket disconnects, Concierge errors with “Remote Chrome connection lost…” so you can re-run after restarting the browser.
...
### Remote Service Mode (`concierge serve`)
...
1. **Start the host**
   ...
   concierge serve
   ```
   Concierge picks a free port, launches Chrome, starts an HTTP/SSE API, and prints:
   ```
   Listening at 0.0.0.0:9473
   Access token: c4e5f9...
   ```
   Use `--host`, `--port`, or `--token` to override the defaults if needed.
   If the host Chrome profile is not signed into ChatGPT, the service opens chatgpt.com for login and exits—sign in, then restart `concierge serve`.
...
3. **What happens**
   ...
   - `concierge serve` logs the DevTools port of the manual-login Chrome (e.g., `Manual-login Chrome DevTools port: 54371`). Runs automatically attach to that logged-in Chrome; you can use the printed port/JSON URL for debugging if needed.

4. **Stop the host**
   - `Ctrl+C` on the VM shuts down the HTTP server and Chrome. Restart `concierge serve` whenever you need a new session; omit `--token` to let it rotate automatically.
