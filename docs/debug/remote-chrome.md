...
- Goal: Run Concierge browser mode from local laptop against VM-hosted Chrome via new `concierge serve` remote service.
- VM IP: 192.168.64.2
...
- `./runner` requires bun; starting service with:
  ...
  ./runner pnpm run concierge -- serve --port 49810 --token cd93955b64d5afcb946a4a4a89651313
  ```
...
- Keychain/cookie read now failing on VM: `loadChromeCookies` throws “Unknown error” when invoked from the server process (Node 25, SSH shell). When `concierge serve` runs from GUI Terminal it starts fine; under nohup/SSH it logs the rejection and remote runs hang.
- New behavior (post-fix): `concierge serve` exits early if it cannot load host ChatGPT cookies after opening chatgpt.com for login; sign in on the host and restart the service.
...
- If cookie loading keeps failing under SSH/nohup, start `concierge serve` from a GUI macOS session or switch to Node 20 to avoid Keychain issues. The service now exits early after opening chatgpt.com when no cookies are present—log in, then restart it.
- Retry local command above; ensure service logs show incoming /runs and that the login probe passes (no login button).
- Optional: switch to a fresh port/token (`concierge serve` with no args) to avoid lingering listeners.
