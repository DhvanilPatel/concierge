...
- Browser engine now works on Linux (Chrome/Chromium/Edge) without the old `DISPLAY` guard. Concierge will launch whatever `chrome-launcher` finds or what you pass via `CHROME_PATH`.
...
- If you use a non-default profile or a custom install, point Concierge at the correct paths:
