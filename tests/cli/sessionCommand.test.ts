...
describe('handleSessionCommand', () => {
  ...
  test('prints paths when --path is provided with an id', async () => {
    ...
    const getSessionPaths = vi.fn().mockResolvedValue({
      dir: '/tmp/.concierge/sessions/abc',
      metadata: '/tmp/.concierge/sessions/abc/meta.json',
      request: '/tmp/.concierge/sessions/abc/request.json',
      log: '/tmp/.concierge/sessions/abc/output.log',
    });
    ...
    expect(logSpy).toHaveBeenCalledWith('Session dir: /tmp/.concierge/sessions/abc');
    expect(logSpy).toHaveBeenCalledWith('Metadata: /tmp/.concierge/sessions/abc/meta.json');
    expect(logSpy).toHaveBeenCalledWith('Request: /tmp/.concierge/sessions/abc/request.json');
    expect(logSpy).toHaveBeenCalledWith('Log: /tmp/.concierge/sessions/abc/output.log');
    expect(process.exitCode).toBeUndefined();
  });
  ...
  test('rejects slug-style "clear" ids with guidance', async () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      'Session cleanup now uses --clear. Run "concierge session --clear --hours <n>" instead.',
    );
    expect(process.exitCode).toBe(1);
  });
});
