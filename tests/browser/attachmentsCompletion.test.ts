...
describe('attachment completion fallbacks', () => {
      ...
      evaluate: vi.fn().mockResolvedValue({
        result: {
          value: {
            ...
            filesAttached: true,
            attachedNames: [],
            inputNames: ['concierge-attach-verify.txt'],
          },
        },
      }),
    } as unknown as ChromeClient['Runtime'];

    const promise = waitForAttachmentCompletion(runtime, 10_000, ['concierge-attach-verify.txt']);
  ...
  test('waitForAttachmentCompletion times out when send button stays disabled (upload likely in progress)', async () => {
      ...
      evaluate: vi.fn().mockResolvedValue({
        result: {
          value: {
            ...
            uploading: false,
            filesAttached: true,
            attachedNames: ['concierge-attach-verify.txt'],
            inputNames: [],
          },
        },
      }),
    ...
    const promise = waitForAttachmentCompletion(runtime, 800, ['concierge-attach-verify.txt']);
...
describe('sent turn attachment verification', () => {
  test('waitForUserTurnAttachments resolves when last user turn includes filename', async () => {
    ...
    await expect(waitForUserTurnAttachments(runtime, ['concierge-attach-verify.txt'], 1000)).resolves.toBe(true);
  });
  ...
  test('waitForUserTurnAttachments times out when filename never appears', async () => {
    ...
    const promise = waitForUserTurnAttachments(runtime, ['concierge-attach-verify.txt'], 600);
  ...
  test('waitForUserTurnAttachments resolves when attachment UI count satisfies expected files (no filename text)', async () => {
    ...
    await expect(
      waitForUserTurnAttachments(runtime, ['concierge-attach-verify-a.txt', 'concierge-attach-verify-b.txt'], 1000),
    ).resolves.toBe(true);
  });
});
