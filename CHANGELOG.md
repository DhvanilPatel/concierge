...
### Changed
- Branding: rename Oracle → Concierge across CLI/docs/tests; environment variables now use `CONCIERGE_*` (legacy `ORACLE_*` removed).
...
### Added
- Config: allow `browser.thinkingTime`, `browser.manualLogin`, and `browser.manualLoginProfileDir` defaults in `~/.concierge/config.json`.
...
- Cookie handling prefers live Chrome over legacy `~/.concierge/cookies.json`; Gemini web can use inline cookies when sync is disabled.
...
- Browser: new `--browser-attachments <auto|never|always>` (default `auto`) pastes file contents inline up to ~60k characters, then switches to uploads; if ChatGPT rejects an inline paste as too large, Concierge retries automatically with uploads.
...
### Changed
- Docs: README now explicitly warns against `pnpx concierge-ai` (pnpx cache breaks sqlite bindings); use `npx -y concierge-ai` instead. Thanks Xuanwo for flagging this.
...
- Browser interruption: if the user/agent sends SIGINT/SIGTERM/SIGQUIT while the assistant response is still pending, Concierge leaves Chrome running, writes runtime hints, and logs how to reattach with `concierge session <slug>` instead of killing the browser mid-run.
...
### Changed
- `concierge` with no arguments now prints the help/usage banner; launch the interactive UI explicitly via `concierge tui` (keeps `CONCIERGE_FORCE_TUI` for automation/tests). README updated to match.
...
### Fixed
- Browser config defaults from `~/.concierge/config.json` now apply when CLI flags are untouched (chromePath/profile/cookiePath), fixing “No Chrome installations found” when a custom browser path is configured.
...
### Changed
- `concierge status` uses the same session table formatting as the TUI (status/model/mode/timestamp/chars/cost/slug) for consistent layout.
...
### Added
- Browser sessions now persist Chrome reattach hints (port/host/target/url) and log them inline; `concierge session <id>` can reconnect to a live tab, harvest the assistant turn, and mark the run completed even if the original controller died. Includes a reconnection helper and regression tests for runtime hint capture and reattach.
...
### Fixed
- CLI/TUI now print the intro banner only once; forced TUI launches (`CONCIERGE_FORCE_TUI` or no args in a TTY) no longer show duplicate 🧿 header lines.
- TUI session list cleans up separators, removing the `__disabled__ (Disabled)` placeholder and `(Disabled)` tag on the header row.
- `concierge session --render` no longer drops answers when the model filter is empty or per-model logs are missing (common for browser runs); stored session output is rendered again.
...
- Browser reattach flow survives controller loss: the controller PID is persisted with the Chrome port/URL so `concierge session <id>` can reconnect, harvest the assistant turn, and mark the run completed even if the original process died.
...
### Added
- `--browser-manual-login` skips cookie copy, reuses a persistent automation profile (`~/.concierge/browser-profile` by default), and waits for manual ChatGPT login—handy on Windows where app-bound cookies can’t be decrypted; works as an opt-in on macOS/Linux too.
- Manual-login browser sessions can reuse an already-running automation Chrome when remote debugging is enabled; point Concierge at it via `--remote-chrome <host:port>` to avoid relaunching/locks.
- `--browser-port` (alias `--browser-debug-port`, env `CONCIERGE_BROWSER_PORT`) pins the DevTools port so WSL/Windows users can open a single firewall rule; includes a lightweight `pnpm test:browser` DevTools reachability check.
...
- Publish metadata now declares Node >=20 (engines/devEngines) and drops the implicit bun runtime so `npx concierge-ai` no longer fails with EBADDEVENGINES on newer Node versions.
...
- Grok “thinking”/non-fast variant is not available via API yet; Concierge aliases `grok` to the fast reasoning model to match what xAI ships today.
- PTY-driven CLI/TUI harness landed for e2e coverage (browser guard, TUI exit path); PTY suites are opt-in via `CONCIERGE_ENABLE_PTY_TESTS=1` and stub tokenizers to stay lightweight.

### Fixed
- MCP (global installs): keep the stdio transport alive until the client closes it so `concierge-mcp` doesn’t exit right after `connect()`; npm -g / host-spawned MCP clients now handshake successfully (tarball regression in 0.4.2).

## 0.4.2 — 2025-11-21

### Fixed
- MCP: `npx concierge-ai concierge-mcp` now routes directly to the MCP server (even when npx defaults to the CLI binary) and keeps stdout JSON-only for Cursor/other MCP hosts.
- Added the missing `@anthropic-ai/tokenizer` runtime dependency so `npx concierge-ai concierge-mcp` starts cleanly.
...
- Duplicate prompt guard remains active: Concierge blocks a second run when the exact prompt is already running.
...
- Reject prompts shorter than 10 characters with a friendly hint for pro-tier models (`gpt-5.1-pro`) only (prevents accidental costly runs while leaving cheaper models unblocked). Override via CONCIERGE_MIN_PROMPT_CHARS for automated environments.
...
- **Gemini 3 Pro Support**: Use Google's latest model via `concierge --model gemini`. Requires `GEMINI_API_KEY`.
...
- Browser inline cookies: `--browser-inline-cookies[(-file)]` (or env) accepts JSON/base64 payloads, auto-loads `~/.concierge/cookies.{json,base64}`, adds a cookie allowlist (`--browser-cookie-names`), and dry-run now reports whether cookies come from Chrome or inline sources.
...
### Changed
 ...
 - Sessions TUI separates recent vs older runs, adds an Older/Newer action, keeps headers aligned with rows, and avoids separator crashes while preserving an always-selectable “ask concierge” entry.
...
### Added
- `concierge-mcp` stdio server (bin) with `consult` and `sessions` tools plus read-only session resources at `concierge-session://{id}/{metadata|log|request}`.
...
- Per-user JSON5 config at `~/.concierge/config.json` to set default engine/model, notification prefs (including sound/mute rules), browser defaults, heartbeat, file-reporting, background mode, and prompt suffixes. CLI/env still override config.
...
- Default ignores when crawling project roots now drop common build/cache folders (`node_modules`, `dist`, `coverage`, `.git`, `.turbo`, `.next`, `build`, `tmp`) unless the path is passed explicitly. Concierge logs each skipped path for transparency.
...
- macOS notifier helper now ships signed/notarized with the Concierge icon and auto-repairs execute bits for the fallback terminal-notifier.
...
- Help banner and first tip now require “prompt + --file” (dirs/globs fine) and remind you Concierge can’t see your project without attachments.
...
- Local stdio (mcporter): add to `config/mcporter.json`
  ```json
  {
    ...
    "type": "stdio",
    "command": "npx",
    "args": ["-y", "concierge-ai", "concierge-mcp"]
  }
  ```
- Claude Code (global/user scope):  
  `claude mcp add --scope user --transport stdio concierge -- concierge-mcp`
- Project-scoped Claude: drop `.mcp.json` next to the repo root with
  ```json
  {
    "mcpServers": {
      "concierge": { "type": "stdio", "command": "npx", "args": ["-y", "concierge-ai", "concierge-mcp"] }
    }
  }
  ```
- The MCP `consult` tool honors `~/.concierge/config.json` defaults (engine/model/search/prompt suffix/heartbeat/background/filesReport) unless the caller overrides them.
...
Highlights
- Markdown rendering for completed sessions (`concierge session|status <id> --render` / `--render-markdown`) with ANSI formatting in rich TTYs; falls back to raw when logs are huge or stdout isn’t a TTY.
- New `--path` flag on `concierge session <id>` prints the stored session directory plus metadata/request/log files, erroring if anything is missing. Uses soft color in rich terminals for quick scanning.

Details
### Added
- `concierge session <id> --path` now prints the on-disk session directory plus metadata/request/log files, exiting with an error when any expected file is missing instead of attaching.
- When run in a rich TTY, `--path` labels and paths are colorized for easier scanning.

### Improved
- `concierge session|status <id> --render` (alias `--render-markdown`) pretty-prints completed session markdown to ANSI in rich TTYs, falls back to raw when non-TTY or oversized logs.
## 0.0.10 — 2025-11-17

### Added
- Rich terminals that support OSC 9;4 (Ghostty 1.2+, WezTerm, Windows Terminal) now show an inline progress bar while Concierge waits for the OpenAI response; disable with `CONCIERGE_NO_OSC_PROGRESS=1`, force with `CONCIERGE_FORCE_OSC_PROGRESS=1`.
...
### Changed
- Help tips call out that Concierge is one-shot and does not remember prior runs, so every query should include full context.
- `concierge session <id>` now logs a brief notice when extra root-only flags are present (e.g., `--render-markdown`) to make it clear those options are ignored during reattach.

## 0.0.7 — 2025-11-16

### Changed
- Browser-mode thinking monitor now emits a text-only progress bar instead of the "Pro thinking" string.
- `concierge session <id>` trims preamble/log noise and prints from the first `Answer:` line once a session is finished.
...
### Added
- Positional prompt shorthand: `concierge "prompt here"` (and `npx -y concierge-ai "..."`) now maps the positional argument to `--prompt` automatically.

### Fixed
- `concierge status/session` missing-prompt guard now coexists with the positional prompt path and still shows the cleanup tip when no sessions exist.

## 0.0.1 — 2025-11-15

### Fixed
- Corrected npm binary mapping so `concierge` is installed as an executable. Published with `--tag beta`.
...
### Fixed
- `concierge status` and `concierge session` no longer demand `--prompt` when used directly.
