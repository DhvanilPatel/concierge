...
phase_artifacts() {
  ...
  local tgz="concierge-ai-${VERSION}.tgz"
  mv "$packed" "$tgz"
...
phase_publish() {
  ...
  run "$RUNNER" npm view concierge-ai version
  run "$RUNNER" npm view concierge-ai time
}

phase_smoke() {
  banner "Smoke test in empty dir"
  local tmp=/tmp/concierge-empty
  rm -rf "$tmp" && mkdir -p "$tmp"
  ( cd "$tmp" && npx -y concierge-ai@"$VERSION" "Smoke from empty dir" --dry-run )
}
...
Phases (run individually or all):
  ...
  smoke      empty-dir npx concierge-ai@<version> --dry-run
