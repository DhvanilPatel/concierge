...
function renderHelpBanner(version: string, colors: HelpColors): string {
  const subtitle = 'Your AI concierge — access the best models for tough questions with code/file context.';
  return `${colors.banner(`Concierge v${version}`)} ${colors.subtitle(`— ${subtitle}`)}\n`;
}

function renderHelpFooter(program: Command, colors: HelpColors): string {
  const tips = [
    `${colors.bullet('•')} Required: always pass a prompt AND ${colors.accent('--file …')} (directories/globs are fine); Concierge cannot see your project otherwise.`,
    `${colors.bullet('•')} Attach lots of source (whole directories beat single files) and keep total input under ~196k tokens.`,
    `${colors.bullet('•')} Concierge starts empty—open with a short project briefing (stack, services, build steps), spell out the question and prior attempts, and why it matters; the more explanation and context you provide, the better the response will be.`,
    `${colors.bullet('•')} Spell out the project + platform + version requirements (repo name, target OS/toolchain versions, API dependencies) so Concierge doesn't guess defaults.`,
