...
const TAGLINES = [
  ...
  'Debugger by day, concierge by night.',
  ...
  'When the other agents shrug, the concierge ships.',
  ...
  'New year, same concierge: resolutions shipped, not wished.',
  ...
  "Workers' Day: let concierge haul the heavy context.",
...
const HOLIDAY_RULES = new Map<string, HolidayRule>([
  ['New year, same concierge: resolutions shipped, not wished.', onMonthDay(0, 1)],
  [
    'Lunar New Year sweep: clear caches, invite good deploys.',
    onSpecificDates(
      [
  ...
  ["Workers' Day: let concierge haul the heavy context.", onMonthDay(4, 1)],
...
export function pickTagline(options: TaglineOptions = {}): string {
  ...
  const override = env?.CONCIERGE_TAGLINE_INDEX;
...
export function formatIntroLine(version: string, options: TaglineOptions = {}): string {
  ...
  if (rich && chalk.level > 0) {
    return `${chalk.bold('🛎️ concierge')} ${version} — ${tagline}`;
  }
  return `🛎️ concierge ${version} — ${tagline}`;
}
