...
export function registerSessionsTool(server: McpServer): void {
  server.registerTool(
    'sessions',
    {
      title: 'List or fetch concierge sessions',
      description:
        'List stored sessions (same defaults as `concierge status`) or, with id/slug, return a summary row. Pass detail:true to include metadata, log, and stored request for that session.',
