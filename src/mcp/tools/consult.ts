...
export function registerConsultTool(server: McpServer): void {
  server.registerTool(
    'consult',
    {
      title: 'Run a concierge session',
      description:
        'Run a one-shot Concierge session (API or browser). Attach files/dirs for context, optional model/engine overrides, and an optional slug. Background handling follows the CLI defaults; browser runs only start when Chrome is available.',
